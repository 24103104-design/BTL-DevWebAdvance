...
export class PhieuMuonService {
  ...
  async findOne(maPhieu: string): Promise<PhieuMuonEntity> {
    const phieu = await this.phieuMuonRepository.findOne({ where: { maPhieu } });
    if (!phieu) {
      throw new NotFoundException(`Không tìm thấy phiếu mượn có mã: ${maPhieu}`);
    }
    return phieu;
  }

  // 4. Cập nhật trạng thái / thông tin phiếu mượn (Update)
  async update(maPhieu: string, data: Partial<PhieuMuonEntity>): Promise<PhieuMuonEntity> {
